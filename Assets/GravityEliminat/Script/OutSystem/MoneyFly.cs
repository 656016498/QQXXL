...
{
    public static MoneyFly instance;
    EEDataManager ee = new EEDataManager();
    public List<Fly> flyDATA;
    [SerializeField]
    ...
    private void Awake()
    {
        instance = this;
        ee.Load();
        flyDATA = ee.GetList<Fly>();

    }

    public float GetMoney() {

        foreach (var item in flyDATA)
        {
            if (LargeCashDataControl.Instance.mData.totalNum<item.Max)
            {
                return item.AD;
            }
        }
    ...
    public void PropPlay(int m, Vector3 startPoint, Vector3 endPoint, Action Oncomplete) {



        this.transform.SetAsLastSibling();
        StartCoroutine(IE_play(m, startPoint, endPoint, ()=> {
            ...
            LargeCashDataControl.Instance. mData.totalNum += GetMoney() / m;
            LargeCashDataControl.Instance.largeCash.Value = LargeCashDataControl.Instance.mData.totalNum;
        }));
